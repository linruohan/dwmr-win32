use windows::{
    core::*,
    Win32::Graphics::{Direct2D::*, DirectWrite::*},
};
...
pub unsafe fn implement_draw_text_box(
    ...
    let text_layout =
        write_factory.CreateTextLayout(text, text_format, max_width / dpi, max_height / dpi)?;
    text_layout.SetFontSize(font_size, text_range)?;
    text_layout.SetFontFamilyName(BAR_FONT, text_range)?;
    text_layout.SetLocaleName(w!("ko_kr"), text_range)?;
    ...
    if let Some(super_text_data) = super_text {
        let super_text_range = DWRITE_TEXT_RANGE {
            startPosition: 0,
            length: super_text_data.len() as u32,
        };
        ...
        super_text_layout.SetFontSize(font_size * 0.8, super_text_range)?;
        super_text_layout.SetFontFamilyName(BAR_FONT, super_text_range)?;
        super_text_layout.SetLocaleName(w!("ko-kr"), super_text_range)?;
        super_text_layout.GetMetrics(&mut super_text_metrics)?;

        render_target_ref.DrawTextLayout(
            Vector2 {
                X: rect.right - super_text_metrics.width,
                Y: 0.0,
            },
            &super_text_layout,
            text_brush,
